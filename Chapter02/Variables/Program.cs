...
int length2 = ((string)name).Length; // Caset name to a string.
...
var fruit = "Apple"; // string values use double-quotes.
...
// Bad use of var because we cannot tell the type, so we should use a specific declaration as shown in the second statement.
...
List<Person> list = new() // Instead of: new List<Person>()
{
